...
def _mathIsEqual(solution, response):
	if solution is None or response is None:
		...
		# even itself
		return False
	return 		_mathChildrenAreEqual(solution.childNodes, response.childNodes) \
...
def _mathChildIsEqual(child1, child2):
	...
	if 		child1.nodeType != child2.nodeType \
		or	len(_importantChildNodes(child1.childNodes)) != len(_importantChildNodes(child2.childNodes)):
		return False
